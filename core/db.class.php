...
class db
{
	 ...
	 */
	public function DBSelectOne($table_name, $select_columns_string, $where_array, $limit_string = "", $inner_tab = "", $inner_col = "")
	{
		// PDO - MySQL
		//printr($where_array);
      ...
      // INNER JOIN
      if ($inner_tab != "") $inner_join = "INNER JOIN $inner_tab ON $inner_tab.id_$inner_tab = $table_name.$inner_col";
      else $inner_join = "";	
      
			// 1) pripravit dotaz s dotaznikama
			$query = "select $select_columns_string from `".$table_name."` $inner_join $where_pom $limit_string;";
			//echo "$query <br/>";
	...
	* Nacist vsechny zaznamy z tabulky.
	 ...
	 * @return mixed
	 */
	 public function DBSelectAll($table_name, $select_columns_string, $where_array, $limit_string = "", $order_by_array = array(), $inner1 = "", $inner2 = "")
	 {
	 	// PDO - MySQL
      ...
      // INNER JOIN
      if ($inner1 != "") $inner_join = "INNER JOIN $inner1 ON $inner1.id_$inner1 = $table_name.$inner1
                                        INNER JOIN $inner2 ON $inner2.id_$inner2 = $table_name.$inner2";
      else $inner_join = "";	 			
	
	 		// 1) pripravit dotaz s dotaznikama
	 		$query = "select $select_columns_string from ".$table_name." $inner_join $where_pom $order_by_pom $limit_string;";
	 		//echo $query;
	 ...
	 *
	 * @param string $table_name - jméno tabulky
	 * @param array $where_id - id ke smazani
	 */
	public function DBDelete($table_name, $where_id)
	{
		// PDO - MySQL
		//printr($where_array);
		...
		// slozit si podminku s otaznikama
			...
			// doplnit slovo where
			$where_pom = "where id_$table_name = $where_id";
